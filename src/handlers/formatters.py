...
class UserFriendlyErrorFormatter:
    ...
    def format_api_timeout_error(
        ...
        return (
            f"⏱️ <b>Сервис временно недоступен</b>\n\n"
            f"🔌 Не удалось получить курс {pair_text} от {api_name}\n\n"
            f"💬 <b>Возможные причины:</b>\n"
            f"• Медленное соединение с интернетом\n"
            f"• Высокая нагрузка на сервер {api_name}\n"
            f"• Технические работы\n\n"
            f"🔄 <b>Попробуйте:</b>\n"
            f"• Повторить запрос через 30-60 секунд\n"
            f"• Проверить соединение с интернетом\n"
    ...
    def format_api_error(
        ...
        if "api key" in error_message.lower() or "unauthorized" in error_message.lower():
            specific_help = (
                f"🔑 Проблема с авторизацией в {api_name}\n"
        ...
        return (
            ...
            f"🔌 Не удалось получить курс {pair_text}\n\n"
            f"{specific_help}\n\n"
    ...
    def format_unexpected_error(
        ...
        return (
            ...
            f"⚠️ Произошла неожиданная ошибка при обработке запроса {pair_text}\n\n"
            f"💬 <b>Описание:</b>\n"
            f"• Ошибка в логике обработки данных\n"
            f"• Непредвиденный формат ответа от сервиса\n"
            f"• Внутренняя ошибка бота\n\n"
            f"🔄 <b>Что делать:</b>\n"
            f"• Попробуйте еще раз (/admin_bot)\n"
