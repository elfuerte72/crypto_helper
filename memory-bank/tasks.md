...
### TASK-PERF-001: Исправление Memory Leak в кэшировании - 🔥 КРИТИЧЕСКИЙ
- [ ] **ОПИСАНИЕ**: Кэш в `fiat_rates_service.py` растет бесконечно, вызывая утечки памяти
- [ ] **ПРИОРИТЕТ**: Критический
- [ ] **ФАЙЛЫ К ИЗМЕНЕНИЮ**:
  - [ ] `src/services/fiat_rates_service.py` - добавить TTL cleanup и ограничение размера кэша
  - [ ] `src/services/cache_manager.py` - создать унифицированный менеджер кэша (новый файл)
  - [ ] `src/config.py` - добавить настройки кэширования
- [ ] **ТРЕБОВАНИЯ**:
  - [ ] Ограничить размер кэша до 100 записей
  - [ ] Добавить автоматическую очистку устаревших записей (TTL cleanup)
  - [ ] Реализовать LRU eviction policy
  - [ ] Мониторинг использования памяти кэшем
- [ ] **ОЖИДАЕМЫЙ РЕЗУЛЬТАТ**: Снижение потребления памяти на 30-40%
- [ ] **ТЕСТИРОВАНИЕ**: Unit тесты для cache cleanup и memory limits

### TASK-PERF-002: Оптимизация API Performance - ⚡ ВЫСОКИЙ
- [ ] **ОПИСАНИЕ**: Ускорение API запросов и уменьшение времени ответа
- [ ] **ПРИОРИТЕТ**: Высокий
- [ ] **ФАЙЛЫ К ИЗМЕНЕНИЮ**:
  - [ ] `src/services/api_service.py` - увеличить connection pooling limits
  - [ ] `src/services/unified_api_manager.py` - создать единый API менеджер (новый файл)
  - [ ] `src/services/rate_preloader.py` - предзагрузка популярных курсов (новый файл)
  - [ ] `src/config.py` - оптимизировать таймауты для production
- [ ] **ТРЕБОВАНИЯ**:
  - [ ] Увеличить connection pool: limit=200, limit_per_host=50
  - [ ] Сократить API timeout с 30s до 10s
  - [ ] Предзагружать популярные пары (USDT/RUB, USD/RUB, EUR/RUB) каждые 2 минуты
  - [ ] Batch API requests для получения нескольких курсов
- [ ] **ОЖИДАЕМЫЙ РЕЗУЛЬТАТ**: Ускорение ответов на 40-60%
- [ ] **ТЕСТИРОВАНИЕ**: Load testing для concurrent requests
